...
Copyright © 2016-2017 Syntrogi (dba Qusp)
...
Directives
Directive are special key names that are interpreted differently by Qascade to provide a number of functionalities. These directives are often not assigned verbatim as key:value pairs. However, similar to other keys:value, they are inherited to subfolder, and may be overwritten by manifest files inside subfolders. 
...
Important Notes:
in Qascade manifest files, all specified paths must use unix-style folder separator ‘/. This is to ensure uniform execution across platforms’. Windows-style folder separator ‘\’ will be parsed incorrectly in YAML.
