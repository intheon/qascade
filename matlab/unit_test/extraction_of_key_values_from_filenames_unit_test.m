...
assert(isempty(cell_out{7}))
assert(isempty(cell_out{8}))
%% deal with the . character (required escaping inside the function)

pattern = 'some*_S[subjectNumber]_T[taskLabel].[ext]';
cell_out = extract_key_values_from_filenames({'sometitle_S12_Tr.set'}, pattern);
assert(strcmp(cell_out{1}('ext'),'set'))
