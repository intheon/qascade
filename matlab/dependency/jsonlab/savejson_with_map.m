...
if ismember(class(item), {'OrderedMap' 'containers.Map'})
	txt=map2json(name,item,level,forceArray, varargin{:});
elseif(iscell(item))
...
%%-------------------------------------------------------------------------
function txt=map2json(name,item,level,forceArray, varargin)
txt={};

itemClass = class(item);
if ~ismember(itemClass, {'OrderedMap' 'containers.Map'})
	error('input is not a Map');
end
dim=size(item);
...
if(~isempty(name)) 
    ...
    if(~isempty(names))
      for e=1:length(names)
	    txt{end+1}=obj2json(names{e},item(names{e}),...
             level+(dim(1)>1)+1+forcearray,varargin{:});
        if(e<length(names))
            txt{end+1}=',';
        end
