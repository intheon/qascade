...
keys = newFolderKeyValues.keysInOrder;
...
keys = fileDirective.extract.keysInOrder;
...
if exist('onlyThisFolderFilekeys', 'var')
    keys = onlyThisFolderFilekeys.keysInOrder;
    for i=1:length(keys)
...
if isempty(map)
    ...
    values = {};
    orderedKeys = map.keysInOrder;
    for i=1:length(orderedKeys)
        values{i} = map(orderedKeys{i});
    end;
    newMap = OrderedMap(map.keysInOrder, values, 'UniformValues', false);
end;
end
